...
// Scrolling slides, for pages the server builds (share links, the live
// viewer, GitHub and Zenodo exports). A slide whose canvas is taller than the
// deck (slide.scrollHeight) is presented one screen at a time and scrolls;
// an element with scrollBehavior 'pin' stays on the screen. The client's
// utils/scrollingSlides.js describes it and has the same code for pages built
// in the browser; the tests check that the two agree.
...
function getScreenCount(slide, slideH) {
  return Math.ceil(getCanvasHeight(slide, slideH) / slideH)
}

const isScrolling = (slide, slideH) => getCanvasHeight(slide, slideH) > slideH

const isPinned = el => el?.scrollBehavior === 'pin'
...
function printScreenBody({ slideW, slideH, canvasH, screen, elementsHtml, pinnedHtml, background = '' }) {
